...
def training(scene, dataset, opt, pipe, dataset_name, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, masks, wandb=None, logger=None):
    ...
    for iteration in range(first_iter, first_iter + opt.iterations + 1):        
        ...
        if args.quantize:
            loss = loss + 0.0 * torch.abs(gaussians._anchor_feat[mask]).mean()
        loss.backward()
    ...
    with torch.no_grad():
        for i, mask in enumerate(masks):
            test(tb_writer, dataset_name, iteration, Ll1, loss, l1_loss, iter_start.elapsed_time(iter_end), 
                    testing_iterations, scene, render, (pipe, background), wandb, logger, mask, eval_lpips=True,
                    dump_images=True, level=args.num_levels-i- 1)
    ...
    with torch.no_grad():
        for i, mask in enumerate(disjointed_masks):
            ...
            scene.gaussians._anchor = scene.gaussians._anchor[mask]
            scene.gaussians._offset = scene.gaussians._offset[mask]
            scene.gaussians._anchor_feat = scene.gaussians._anchor_feat[mask]
    ...
    with torch.no_grad():
        ...
        def _concat_gaussians_(dst, src):
            dev = dst._anchor.device
            dst._anchor          = torch.cat([dst._anchor,           _to_dev(src._anchor, dev)], dim=0)
            dst._offset       = torch.cat([dst._offset,       _to_dev(src._offset, dev)], dim=0)
            dst._anchor_feat      = torch.cat([dst._anchor_feat,      _to_dev(src._anchor_feat, dev)], dim=0)
        ...
        scene.gaussians = acc
        test(tb_writer, dataset_name, iteration, Ll1, loss, l1_loss, iter_start.elapsed_time(iter_end), 
                    testing_iterations, scene, render, (pipe, background), wandb, logger, None, eval_lpips=False,
                    dump_images=False, level=args.num_levels-i- 1)
    ...
    #             end = time.time() - start
    #             coding_times.append(end)
            
    #         for path in save_paths:
    #             start = time.time()
    #             gaussians.load_npz(path)
    #             end = time.time() - start
    #             decoding_times.append(end)
    ...
    #         write(f'coding_times: {coding_times}')
    #         write(f'decoding_times: {decoding_times}')
...
def iterative_masking(args_param, dataset, opt, pipe, dataset_name, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, wandb=None, logger=None):
    ...
    gaussians.load_ply_sparse_gaussian(os.path.join(args.pretrained_path, 'point_cloud', f'iteration_{args.load_iter}', 'point_cloud.ply'))
    gaussians.load_mlp_checkpoints(os.path.join(args.pretrained_path, 'point_cloud', f'iteration_{args.load_iter}'))
    ...
    k_s = list(np.flip((num_offsets - geometric_progression(args.min, num_offsets*tmux a args.max, args.num_levels))))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--pretrained_path", type=str, default=None, help='pre-trained model root')
    parser.add_argument("--load_iter", type=int, default=30_000, help='iteration of the pre-trained model to load')
    parser.add_argument("--scene", type=str, default=None)
    parser.add_argument("--num_levels", type=int, default=8, help='number of levels')
    parser.add_argument("--min", type=int, default=50_000, help='minimum number of anchor points of the lowest lod')
    parser.add_argument("--max", type=float, default=0.85, help="maximum number of anchor point of the highest level (0, 1]")
    parser.add_argument("--weighted_sampling", action='store_true', default=False, help='weighted sampling weights. the higher, the more higher LoD will be sampled during fine-tuning')
    parser.add_argument("--G", type=int, default=4)
    parser.add_argument("--quantize", action='store_true', default=False, help='enable quantization-aware fine-tuning (QAT)')
