...
def create_videos(base_dir, input_dir, out_name, num_frames=480, fps=24, payload=None, i=None):
    video_prefix = f"{out_name}"
    os.makedirs(base_dir, exist_ok=True)

    renders_dir = os.path.join(input_dir, "renders") if i == None else os.path.join(input_dir, f"renders_{i}")
    zpad = max(5, len(str(num_frames - 1)))
    idx_to_str = lambda idx: str(idx).zfill(zpad)
    ext = "png"
    ...
    def to_rgb(img):
        ...
        if img.ndim == 2:
            ...
            if img.shape[2] == 4:
                ...
                raise ValueError(f"Canali non supportati: shape={img.shape}")
        else:
            raise ValueError(f"Dimensioni non supportate: shape={img.shape}")
        return img

    first_path = os.path.join(renders_dir, f"{idx_to_str(0)}.{ext}")
    if not os.path.exists(first_path):
        return

    img0 = load_img(first_path)
    img0 = to_rgb(img0)
    h, w = img0.shape[:2]
    ...
    try:
        for idx in tqdm(range(num_frames)):
            img_path = os.path.join(renders_dir, f"{idx_to_str(idx)}.{ext}")
            ...
            if img.shape[:2] != (h, w):
                raise ValueError(f"Frame {idx} ha shape {img.shape[:2]}, atteso {(h, w)}")
